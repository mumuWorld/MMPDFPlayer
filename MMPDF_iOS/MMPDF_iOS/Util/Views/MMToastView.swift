...
class MMToastView: UIView {
    ...
    class func show(message: String, deadTime: CGFloat = 1.0, position: MMToastPostion = MMToastPostion.Center) -> Void {
        ...
        let maxWidth = kScreenWidth - 40 - 20;
        ...
        toastView.backgroundColor = MMColors.color_bg_1?.withAlphaComponent(0.8)
        //设置frame
        toastView.mm_width = min(maxWidth + 20, fitSize.width + 20)
        toastView.mm_height = fitSize.height + 20
        if position == .Center {
            toastView.mm_centerX = kScreenWidth * 0.5
            toastView.mm_centerY = kScreenHeigh * 0.5
        ...
        tWindow!.checkSubViewRemove(checkViewStr: "MMToastView")
